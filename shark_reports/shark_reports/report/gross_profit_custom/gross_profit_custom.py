...
def get_data_when_grouped_by_invoice(columns, gross_profit_data, filters, group_wise_columns, data):
	...
	columns[2] = "Sales Invoice:Link/Item:300"
	...
	for src in gross_profit_data.si_list:
		...
		src.sales_order_date = frappe.db.get_value("Sales Order", src.sales_order, "transaction_date") if src.sales_order else None
		src.sales_qty = frappe.db.get_value("Sales Order Item", src.so_detail, "qty")
		src.sales_amount = frappe.db.get_value("Sales Order Item", src.so_detail, "base_amount")
		src.pincode = frappe.db.get_value("Address", src.shipping_address_name, "pincode") if src.shipping_address_name else None
		src.dispatch_date = src.posting_date
		src.project_name = frappe.db.get_value("Sales Order", src.sales_order, "project") if src.sales_order else None
		src.project_id = frappe.db.get_value("Project", src.project_name, "name") if src.project_name else None
		src.project_format_ = frappe.db.get_value("Sales Order", src.sales_order, "project_format_") if src.sales_order else None

		so_qty += src.sales_qty if src.sales_qty else 0
	...
	column_map = frappe._dict(
		{
			...
			"project_format_": {
				"label": _("Project Format"),
				"fieldname": "project_format_",
				"fieldtype": "Data",
				"width": 100,
			},
			"sales_qty": {
	...
	return frappe._dict(
		{
			...
			"project_format_": "project_format_",
			"sales_qty": "sales_qty",
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql(
			...
			select
				...
				`tabSales Invoice`.shipping_address, `tabSales Invoice`.shipping_address_name
				{sales_person_cols}
				{payment_term_cols}
			from
